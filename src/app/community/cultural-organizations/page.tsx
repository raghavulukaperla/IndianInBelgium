import type { Metadata } from "next";
import { ListPageTemplate } from "@/components/content/list-page-template";
import { CommunityOrgCard } from "@/components/content/cards/community-org-card";
import { buildMetadata } from "@/lib/metadata";
import { getCommunityOrgs } from "@/lib/data";

const data = getCommunityOrgs();
const items = data.items.filter((i) => i.type === "cultural-org");

export const metadata: Metadata = buildMetadata({
  title: "Cultural Organizations",
  description: "Arts, dance, music and cultural groups in Belgium.",
  path: "/community/cultural-organizations",
});

export default function CulturalOrganizationsPage() {
  return (
    <ListPageTemplate
      title="Cultural Organizations"
      description="Arts, dance, music and cultural groups in Belgium."
      lastUpdated={data.lastUpdated}
      isSampleData={data.isSampleData}
      items={items}
      keyFor={(i) => i.id}
      renderItem={(i) => <CommunityOrgCard item={i} />}
    />
  );
}
